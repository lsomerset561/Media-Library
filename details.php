...
if (isset($_GET["id"])) { //if ID passed
  $id = $_GET["id"];
  if (isset($catalog[$id])) { //if ID in catalog
    $item = $catalog[$id];
  }
}

if (!isset($item)) { //if item not found or $item not set
  header("location:catalog.php"); //redirect to full catalog page
  exit; //makes sure nothing else processed while redirect occurs
}
